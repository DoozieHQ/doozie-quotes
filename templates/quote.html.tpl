...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .thumb{width:100%;height:260px;object-fit:cover;border-radius:var(--rounded);border:1px solid var(--mid);cursor:pointer;transition:.25s;}
    ...
    /* IMPORTANT: give the viewer a box so the absolutely-positioned iframe can fill it */
    .viewer{
      position:relative;
      height:520px;             /* adjust to taste; or replace with aspect-ratio below */
    ...
    .fab-3d{
      ...
      background: var(--white);
      color: var(--black);
      font-size: 20px;           /* sharper icon at 48px */
      display: flex;
      align-items: center;
      justify-content: center;
      cursor: pointer;
      z-index: 3;                /* above shield & iframe */
    ...
    /* Click-shield — bigger than the FAB to blanket the embed’s button below (z:2) */
    .fab-shield{
      position: absolute;
      /* Slightly larger and inset a tiny bit less to completely cover the embed FAB */
      bottom: 14px;              /* 2px tighter than our FAB for more coverage */
      right: 14px;
      width: 56px;               /* bigger than our 48px button */
      height: 56px;
      z-index: 2;                /* between our FAB and the iframe */
      background: transparent;   /* invisible */
      pointer-events: auto;      /* ensure it soaks up stray clicks */
      /* (Optional) If you ever need to debug coverage, temporarily add:
        background: rgba(255,0,0,0.15);
      */
    }

    /* Safety: make sure the viewer box exists so our absolute elements have a reference */
    .viewer-wrap{ position: relative; }
    .viewer{ position: relative; }  /* already set in your template */
    ...
    .swatch-thumb{width:100%;aspect-ratio:1;object-fit:cover;border-radius:12px;border:1px solid var(--mid);cursor:pointer;transition:.25s;}
  ...
  <!-- External script (deferred so it runs after parsing even if </body> is missing) -->
  <script defer src="/assets/quote-page.js"></script>
</head>
...
<body>
  ...
  <div class="wrap">
    ...
    <div class="section">
      <h2>Main Views</h2>
      <p style="color:var(--grey);font-size:0.9rem;">Tap to view full size</p>
    ...
    <div class="section">
      ...
      <p style="color:var(--grey);font-size:0.9rem;">Rotate, zoom and pan to explore the model.</p>

      <div class="viewer-wrap">
        <div id="viewerInline" class="viewer">
          {{THREED_IFRAME_URL}}
        </div>

        <button class="fab-3d" id="open3dFullscreen" type="button" aria-label="Open 3D Viewer Fullscreen">⤢</button>
    ...
    <!-- FOOTER -->
    <div class="footer">© 2026 • YOUR_COMPANY_NAME • YOUR_COMPANY_ADDRESS • SALES_EMAIL</div>
