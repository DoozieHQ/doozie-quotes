...
function build3DIframe({ owner, repo, branch, leadId, viewerRelPaths }) {
...
(async function main() {
  ...
  const viewerDir  = path.join(localAssets, 'viewer');
  ...
  // Discover files
  const viewerFilesAbs = await listFilesAbs(viewerDir, ['.3ds','.png','.jpg','.jpeg']);
  ...
  // 3D iframe HTML (complete, valid)
  const THREED_IFRAME_URL =
    viewerRelPaths.length
      ? build3DIframe({ owner: GH_OWNER, repo: GH_REPO, branch: GH_BRANCH, leadId, viewerRelPaths })
      : '';
