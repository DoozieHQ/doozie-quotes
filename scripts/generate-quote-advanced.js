...
function build3DViewerIframe(owner, repo, branch, filePaths) {
  ...
  // 3dviewer *embed* endpoint supports comma-separated file URLs in model=
  const modelParam = encodeURIComponent(urls.join(','));
  return `<iframe src="https://3dviewer.net/embed.html#model=${modelParam}" allowfullscreen></iframe>`;
}
