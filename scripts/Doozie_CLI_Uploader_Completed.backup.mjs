...
async function normalizeModelFilename(viewerDir) {
  // Find exactly one .3ds file (you already validate this in your flow)
  const files = await fs.readdir(viewerDir);
  const models = files.filter(f => f.toLowerCase().endsWith(".3ds"));
  if (models.length !== 1) return; // nothing to do here, your validator will catch if it's wrong

  const current = models[0];

  // Only normalize if there are spaces; leave short names and textures untouched.
  if (!/\s/.test(current)) return;

  const normalized = current.replace(/\s+/g, "_"); // "Study V4 - Doors Off.3ds" -> "Study_V4_-_Doors_Off.3ds"
